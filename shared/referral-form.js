...
export function generateReferralForm(data) {
  ...
  const audiogramSVG = buildAudiogramSVG(thresholds, baseline)
  const thresholdBoxes = buildThresholdBoxes(thresholds)
...
<body>
  ...
  <div class="form-footer">
    This referral was generated by HCP-Web · ${esc(org?.name ?? '')} · ${new Date().toLocaleDateString('en-CA')}
  </div>
...
function buildAudiogramSVG(thresholds, baseline) {
  const W = 520, H = 200
  ...
  function plotEar(ear, color, symbol) {
    ...
    const points = keys.map((key, i) => {
      ...
      const db  = val === 'NR' ? 105 : (val != null ? Number(val) : null)
      if (db === null) return null
      return { x: xPos(i), y: yPos(Math.min(db, DB_MAX)), db }
    })
    ...
    const dots  = validPoints.map(p =>
      symbol === 'O'
        ? `<circle cx="${p.x}" cy="${p.y}" r="4" fill="white" stroke="${color}" stroke-width="1.5" />`
        : `<text x="${p.x}" y="${p.y + 4}" text-anchor="middle" font-size="10" fill="${color}" font-weight="700">X</text>`
    ).join('')
...
function buildThresholdBoxes(thresholds) {
  return `
          ...
          <div class="threshold-freq-row">
            ${FREQ_KEYS[ear].map((key, i) => {
              ...
              return `<div class="threshold-cell">
                ...
                <span class="freq-val">${esc(display)}</span>
              </div>`
            }).join('')}
          </div>
        </div>
